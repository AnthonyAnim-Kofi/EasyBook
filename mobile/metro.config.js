...
const polyfillOrigins = [
  path.resolve(__dirname, './polyfills/native'),
  path.resolve(__dirname, './polyfills/web'),
  path.resolve(__dirname, './polyfills/shared'),
].map(normalizePath);
...
config.resolver.resolveRequest = (context, moduleName, platform) => {
  try {
    // Avoid re-aliasing imports inside our own polyfills, especially on Windows where
    // path separators can otherwise make the guard miss and create self-import cycles.
    const originModulePath = normalizePath(context.originModulePath);
    if (polyfillOrigins.some((polyfillOrigin) => originModulePath.startsWith(polyfillOrigin))) {
